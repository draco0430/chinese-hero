...
func (h *DonateGoldToGuildHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if s.Character.Gold < gold {
		resp := messaging.InfoMessage("You don't have enough gold.")
		return resp, nil
	}
	...
	//guild.GuildDonateCalculate()

	guild.InformMembers(s.Character)
	s.Character.Socket.Write(s.Character.GetGold())
...
func (h *CreateGuildHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	} else if s.Character.Level < 25 {
		msg := "Your level must be more than 2Dan5Kyu to create a house."
		return messaging.InfoMessage(msg), nil
	...
	isValid, _ := regexp.MatchString(validPattern, name)
...
func (h *GuildRequestHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if guild.MemberCount >= int16(maxMemberCount) { // max members reached
...
func (h *ExpelFromGuildHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	c.GuildID = -1
	c.ClanGoldDonation = 0
	//c.RemoveGuildTierBuffs(int(guild.Recognition))
	go c.Update()
...
func (h *LeaveGuildHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if guild.LeaderID == s.Character.ID { // dissolve guild
		...
		s.Character.ClanGoldDonation = 0
		//s.Character.RemoveGuildTierBuffs(int(guild.Recognition))
		spawnData, err := s.Character.SpawnCharacter()
		if err == nil {
			p := nats.CastPacket{CastNear: true, CharacterID: s.Character.ID, Type: nats.PLAYER_SPAWN, Data: spawnData}
			p.Cast()
