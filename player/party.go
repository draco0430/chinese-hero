...
func (h *SendPartyRequestHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if funk.Contains(database.LoseEXPServers, int16(s.User.ConnectedServer)) {
		return nil, nil
	}
	...
	//resp[9] = byte(s.Character.PartyMode)
...
func (h *ChangePartyModeHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	resp := utils.Packet{0xaa, 0x55, 0x05, 0x00, 0x72, 0x0d, 0x0a, 0x00, 0x55, 0xaa}
	pmode := data[6]
	s.Character.PartyMode = int(pmode)
	resp.Insert([]byte{data[6]}, 8)
	return resp, nil
}
*/
...
func (h *ExpelFromPartyHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if s.Character.Map == 243 || s.Character.IsDungeon {
		return nil, nil
	}
