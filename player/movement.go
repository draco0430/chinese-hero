...
func (h *MovementHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	*/
	...
	if c.DuelID > 0 {
		player, _ := database.FindCharacterByID(c.DuelID)
		if player != nil {
			duelPlayerLocation := database.ConvertPointToLocation(player.Coordinate)
			thisPlayerLocation := database.ConvertPointToLocation(c.Coordinate)
			distance := utils.CalculateDistance(duelPlayerLocation, thisPlayerLocation)
			if distance > 30 {
				c.DuelID = 0
				player.DuelID = 0
				c.DuelStarted = false
				player.DuelStarted = false
				c.Socket.Write(database.PVP_FINISHED)
				player.Socket.Write(database.PVP_FINISHED)
			}
		}
	}
	...
	/*
		if c.Map == 254 {
			if coordinate.X > 227 && coordinate.X < 253 && coordinate.Y > 250 && coordinate.Y < 280 {
				coordinate := &utils.Location{X: 261, Y: 245}
				gomap, _ := c.ChangeMap(254, coordinate)
